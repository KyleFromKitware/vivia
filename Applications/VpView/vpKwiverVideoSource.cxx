...
{
  const auto totalFrames = static_cast<ssize_t>(this->FramePaths.size());
...
{
  if (frame < 0 || frame > static_cast<ssize_t>(this->num_frames()))
    {
    return false;
    }

  this->CurrentFrame = static_cast<ssize_t>(frame);
  ts = this->frame_timestamp();
